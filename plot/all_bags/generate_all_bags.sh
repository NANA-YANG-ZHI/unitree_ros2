...
# example/data/2026_07_07/usable_data, saving all derived data in
# plot/all_bags/ (kept separate from plot/contact_estimation/, which holds
# only code: plot_contact_estimate.py, debug/, validation/, tutorial/).
#
# Runs entirely in unitree_ros2_dev (source ROS2 + cyclonedds setup first).
# Per bag:
#   1. bag_topic_to_npz.py       -> <bag>_lowstate.npz, <bag>_sportmodestate.npz
#   2. compute_gt_contact_signals.py -> <bag>_gt_contact_comparison.npz
#   3. run_contact_estimation.py -> <bag>_contact_estimate.npz
#
...
for BAG in "${BAGS[@]}"; do
    ...
    docker exec -w /workspace "$PINOCCHIO_CONTAINER" bash -c "
        ...
        cd example/src/src/contact_estimation

        python3 bag_topic_to_npz.py '/workspace/${BAG_PATH}' /lowstate \
            --out '/workspace/${OUT_DIR}/${BAG}_lowstate.npz'
        python3 bag_topic_to_npz.py '/workspace/${BAG_PATH}' '${SPORTMODE_TOPIC}' \
            --out '/workspace/${OUT_DIR}/${BAG}_sportmodestate.npz'

        python3 compute_gt_contact_signals.py \
            '/workspace/${OUT_DIR}/${BAG}_lowstate.npz' \
            '/workspace/${OUT_DIR}/${BAG}_sportmodestate.npz' \
            --out '/workspace/${OUT_DIR}/${BAG}_gt_contact_comparison.npz'

        python3 run_contact_estimation.py '/workspace/${BAG_PATH}' \
            --out '/workspace/${OUT_DIR}/${BAG}_contact_estimate.npz'
    "
done

echo "Done. All derived data saved under ${OUT_DIR}/"
