// Walk forward while oscillating body pitch in a sinusoidal pattern via Euler().
...
static constexpr double SETTLE_TIME  = 1.0;    // s: reach center pitch before walking
static constexpr double WALK_TIME    = 5.0;    // s: walk with sin-wave pitch (1 full cycle)
static constexpr double RESTORE_TIME = 1.0;    // s: stop and restore zero pitch
...
class WalkWithSinPitch : public rclcpp::Node
{
public:
    WalkWithSinPitch() : Node("walk_with_sin_pitch"), t_(-1.0)
    {
        state_sub_ = create_subscription<unitree_go::msg::SportModeState>(
            "sportmodestate", 10,
            std::bind(&WalkWithSinPitch::state_cb, this, _1));
        ...
        timer_ = create_wall_timer(
            ...
            std::bind(&WalkWithSinPitch::control_cb, this));
    }
...
private:
    ...
    void control_cb()
    {
        ...
        if (t_ < SETTLE_TIME) {
            sport_req_.Euler(req, 0.0f, deg2rad(PITCH_CENTER_DEG), 0.0f);
            ...
            sport_req_.Euler(req,  0.0f, deg2rad(pitch_deg), 0.0f);
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::spin(std::make_shared<WalkWithSinPitch>());
    rclcpp::shutdown();
    return 0;
}
